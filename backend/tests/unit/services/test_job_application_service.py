...
def repo():
    ...
    repo.list_with_offer = AsyncMock()
    repo.get_by_id_with_offer = AsyncMock()
    return repo
...
# --- create_application ---
@pytest.mark.asyncio
async def test_create_application_raises_if_job_offer_not_found(
    service: JobApplicationService,
    session,
):
    # Mock FK check: no job offer
    result = MagicMock()
    ...
    assert exc.value.detail == "Job offer not found"
...
async def test_create_application_success(
    ...
    # Mock FK check: job offer exists
    result = MagicMock()
...
async def test_list_applications_calls_repo(
    ...
    repo.list_with_offer.return_value = expected

    result = await service.list_applications()

    assert result == expected
    repo.list_with_offer.assert_awaited_once()
...
async def test_update_application_raises_if_not_found(
    ...
    repo.get_by_id_with_offer.return_value = None
...
async def test_update_application_success(
    ...
    repo.get_by_id_with_offer.return_value = job_application
